...
- 狀態: ✅(2026-08-02 完成,測試累計 141 passed,並在瀏覽器以雙語實際瀏覽驗證)
...
- 實作:
  ...
  - **後台整個移到 `/admin/*`,根路徑讓給官網**,產品因此能放在大家預期的網址上
  - `PublicProductController` 與後台控制器分開,而不是在同一支裡開一條公開分支
  - `PublicProductResource` 是白名單:slug / 名稱 / 摘要 / 封面 / 發佈日期,
    內文只在詳情頁給。沒有 id、沒有 status、沒有 sort_order
  - 草稿回傳 **404,且與「這個 slug 從來不存在」的回應一字不差**
  - `PublicLayout` 自己的頁首頁尾、語言切換,已登入者有回後台的入口
  - 詳情頁用 `v-html` 渲染編輯器產出的 HTML —— 這安全的唯一理由是寫入時已淨化
  - `usePageMeta` 讓 document title 與 meta description 跟著頁面走
...
**後台得先搬家,官網才做得起來**
兩邊都想要 `/products`,而官網的理由比較硬:那是訪客會猜的網址,也是會出現在搜尋結果裡的那個。
所以後台整個搬到 `/admin/*`。這不只是換路徑 —— 網址現在直接說出了那條界線,
「`/admin` 底下都要帳號」變成看網址列就能確認的規則,而不是要去讀 router 才知道。
路由名稱沒變,所以用名稱連結的地方一行都不用改。

**兩支控制器,而不是一支裡開公開分支**
`ProductController` 的職責是把該給的全部給有權限的人;`PublicProductController` 的職責是
把幾乎所有東西擋在外面。這是兩個相反的預設值,揉進同一個 class 就是草稿哪天流到官網上的原因 ——
因為某個 scope 在五個地方裡加了四個,而第五個是上禮拜才長出來的。

**公開資源用白名單,決定了出錯時往哪個方向倒**
`PublicProductResource` 是列出要輸出的欄位,而不是隱藏不想輸出的欄位。
以後多加一個欄位 —— 內部備註、成本價、供應商 —— 在有人決定要公開之前,它在官網上都是不存在的。
反過來寫的話是預設就漏,要有人記得才不漏。

**草稿要回 404 而不是 403**
403 等於說「這個東西存在,但你不能看」,而那正是未發佈的產品最不該說出口的事 —— 同業也會讀網址。
測試直接斷言兩個回應完全相同,並把 `app.debug` 關掉,比對的才是正式環境真正送出的內容。
開著 debug 的話兩者會差在堆疊追蹤的行號上 —— 那個測試會對一個其實成立的性質說謊。

**這裡的 `v-html` 是「寫入時淨化」的回報**
把編輯器產出的 HTML 渲染在一個對所有人開放的頁面上,正是儲存型 XSS 從「某人的問題」
變成「所有人的問題」的那個點。它安全的唯一理由,是 `App\Support\RichText` 在好幾週前、
這個頁面還不存在的時候,就已經在入口把它洗乾淨了。
一句話的論證:**在入口做的決定,讓所有出口都安全,包含當時還沒想到的出口。**

**切換語言必須重新抓資料,不是重新渲染**
API 是依 `Accept-Language` 回傳對應翻譯,所以產品名稱在回應裡,不在前端的語系檔裡。
切語言就必須發新請求;只重新渲染的元件會在已翻譯的標題底下繼續顯示舊語言的內文,
看起來像快取 bug,實際上是資料流的問題。

**SPA 的 SEO 要誠實**
`usePageMeta` 會設定 title 與 description,這修好了瀏覽器分頁、書籤,以及會執行 JavaScript
的爬蟲。但它不會讓這個站變成伺服器渲染,不跑 JS 的爬蟲看到的仍然是空殼。
真的要做 SEO 需要 SSR 或預渲染 —— 這裡把它寫成「已知限制」,而不是假裝已經解決。

**內文排版自己寫,不裝 typography plugin**
淨化白名單只有 17 個元素,所以編輯器能產出的東西是有邊界而且很少的一組。
文章樣式表裡每一條規則都對應到一個真的會出現的標籤,比通用外掛要處理的清單短得多,
也就不需要多一個相依套件。
